
public class ConvercionDatos {
	
	public static void main(String[] args) {
		//conversiones automaticas de tipos de datos
		byte numeroByte = 10;
		
		short numeroShort = numeroByte;
		
		int numeroInt = numeroShort;
		
		long numeroLong = numeroInt;
		
		int otroInt = (int) numeroLong;//cambiar datos grandes a datos chicos
		
		short otroShort = (short) otroInt;
		
		short numero1 = 123;
		short numero2 = 7343;
		byte numero3 = 12;
		byte numero4 = 33;
		
		//java tiene prioridad con el dato int por eso abara que especificar el tipo de dato en algunos casos
		short suma = (short) (numero1 +  numero2);
		byte suma2 = (byte) (numero3 + numero4);
		short suma4 = (short) (numero1 + numero3);
		
	}

}
